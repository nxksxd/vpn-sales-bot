...
TEMPLATES = {
    ...
    "purchase_success": (
        ...
        "Откройте «\U0001f511 Мои подписки» → «Показать ключ VLESS», чтобы получить ссылку."
    ),
    "referral_bonus": (
...
class NotificationService:
    ...
    async def was_sent(
        ...
        stmt = select(Notification).where(
            ...
            Notification.type == template_key,
        )
        if subscription_id is not None:
            stmt = stmt.where(Notification.subscription_id == subscription_id)
        if since is not None:
            stmt = stmt.where(Notification.sent_at >= since)
    ...
    async def send(
        ...
        **kwargs: str,
        ...
        try:
            ...
            logger.warning(
                "Failed to send notification to {} template={} error={}",
                telegram_id,
                template_key,
                e,
            )
